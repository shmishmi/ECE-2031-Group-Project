...
;* Circle code
;***************************************************************
;this orients the robot the way we want it for the circle (no smacking into the wall)
LOADI 180        
STORE ANGLE
CALL turn

;show a couple circles
CALL Circle
CALL HOLD
CALL Circle
...
;make sure it gets back to 0,0 if it's a little off (circle overshoot)
LOADI 0
STORE X
STORE Y
CALL goto
...
;initialize variables (for repeated calls)
    LOADI -1    ;start atan variable init
    STORE INDEX
    ADDI 1
    STORE YBIG
    ADDI 1
...
IN XPOS            ;change x and y so that they are relative to your current location
...
;this is if you don't want to use relative position
;LOAD Y 
;STORE ytemp
;LOAD X 
;STORE xtemp
...
;find quadrant
    LOAD MaskSign       ;xy info
    AND  ytemp         ;Get sign bit of y
    STORE signcheck

    ;If y is positive or zero go to x
    ...
    ;If y is negative take abs. value,and go to quad 3 (because y is negative in quad 3).          
    MULT NegOne
    STORE ytemp
    LOADI 3
    STORE quadrant
...
quadx:
    ...
    XOR signcheck ;Check to see if sign of y and x are the same sign
    ;Figure out quadrant
    JZERO checkx ; x and y are both the same sign
   
    LOADI 1 ; different signs add one to quadrant
    ADD quadrant 
    STORE quadrant


;Make x positive if not already, needed for actual function
checkx:
	LOAD xtemp
	JPOS compxy
	MULT NegOne
...
compxy:
	...
	;Switch X and Y if Y is bigger need for 90 - atan(x/y)
	LOADI 1
	STORE YBIG
	LOAD xtemp
	STORE TEMP
...
indexloop: 
	...
	LOAD YBIG       ;switch to correct octant
	JZERO quadcalc
...
CW:       ;wheel speeds if you're turning CW
	...
	STORE right
	LOAD ANGLE
	ADDI 4          ;adjustment for turning overshoot
	STORE ANGLE
	JUMP anglemod
...
HOLD:   CALL stopmoving
	...
	JPOS HOLD
        RETURN
;some example shapes that are selected via switch input     
SHAPES:
IN XIO
AND MASK1
JZERO keepshapin
